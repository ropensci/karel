...
They involve a logical evaluation, and if the result is **`TRUE`**, the actions enclosed by this structure are executed. They are expressed in R with the following syntax:
...
The variable **`i`** is called the iteration variable. We could have chosen another letter or word instead, but using **`i`** is quite common. In this example, its sole purpose is to guide the series of steps. The block of instructions is repeated as many times as it takes for **`i`** to reach 5 starting from 1. In general, the syntax for this type of structure is:
